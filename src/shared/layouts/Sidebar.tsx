/**
 * @file Sidebar.tsx
 * @description Sidebar หลักของระบบ ERP แสดงเมนูนำทางทั้งหมด (สไตล์ Modern Premium - Corporate Harmonized)
 * @features
 * - Glassmorphism & Adaptive Theme ที่กลมกลืนกับธีมหลักของโมดูล ERP (ใช้โทนสี Gray และ Royal Blue)
 * - Floating Pill-shaped Menu Items สี Royal Blue (#0055A4) ตรงตามอัตลักษณ์องค์กร
 * - Collapsible submenus พร้อมเส้นโยงระดับโครงสร้างสีเทาบางเฉียบ
 * - Interactive User Profile Card พร้อมสถานะ Online Pulse สี Emerald
 */
        ...
        if (hasSubItems) {
            return (
                <div key={item.id} className="mb-1 mx-3">
                    <div
                        ...
                        className={`
                            ...
                            ${isActive
                                ...
                                : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100/70 dark:hover:bg-gray-800/40 hover:translate-x-1'
                            }
                        `}
                    >
                        <div className="flex items-center space-x-2.5 min-w-0 flex-1 overflow-hidden">
                            {item.icon && (
                                <div className={`flex-shrink-0 transition-transform duration-300 group-hover/item:scale-110 ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-400'}`}>
                                    <item.icon size={18} />
                                </div>
                            )}
                        ...
                        <ChevronDown
                            size={14}
                            className={`flex-shrink-0 text-gray-400 dark:text-gray-500 transition-transform duration-300 ml-1 ${isExpanded ? 'rotate-0' : '-rotate-90'}`}
                        />
                    </div>
                    ...
                    >
                        <div className="ml-5 border-l border-gray-200 dark:border-gray-800 pl-2 space-y-1 py-0.5">
                ...
                <Link
                    ...
                    className={`
                        ...
                        ${isCurrentPage
                            ...
                            : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100/70 dark:hover:bg-gray-800/40 hover:text-gray-900 dark:hover:text-white hover:translate-x-1'
                        }
                    `}
                >
                    <div className="flex items-center space-x-2.5 min-w-0 flex-1 overflow-hidden">
                        {item.icon && (
                            <div className={`flex-shrink-0 transition-transform duration-300 group-hover/link:scale-110 ${isCurrentPage ? 'text-white' : 'text-gray-400 dark:text-gray-400'}`}>
                                <item.icon size={18} />
                            </div>
                        )}
                        <span className="truncate flex-1 leading-snug">{displayLabel}</span>
