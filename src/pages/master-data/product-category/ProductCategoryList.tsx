...
export default function ProductCategoryList() {
    ...
    const fetchData = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            if (searchTerm) {
                ...
                filtered = filtered.filter(i => i.category_code.toLowerCase().includes(term) || i.category_name.toLowerCase().includes(term));
            }
            setItems(filtered);
            setIsLoading(false);
        }, 300);
    ...
    const handleDelete = (id: string) => { if (confirm('คุณต้องการลบข้อมูลหมวดสินค้านี้หรือไม่?')) { console.log('Delete:', id); fetchData(); } };
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><Tag className="text-blue-600" />กำหนดรหัสหมวดสินค้า</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1">จัดการข้อมูลหมวดสินค้าทั้งหมด</p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleCreateNew} className={`${styles.btnPrimary} flex items-center gap-2 whitespace-nowrap`}><Plus size={20} />เพิ่มหมวดสินค้าใหม่</button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input type="text" placeholder="ค้นหารหัสหรือชื่อหมวด..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`${styles.input} pl-10`} />
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className={styles.tableHeader}>
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-200 uppercase">รหัสหมวด</th>
                                <th className="px-6 py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-200 uppercase">ชื่อหมวด</th>
                                <th className="px-6 py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-200 uppercase hidden md:table-cell">ชื่อหมวด (EN)</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {paginatedData.length > 0 ? paginatedData.map((item) => (
                                <tr key={item.category_id} className={styles.tableTr}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600 dark:text-blue-400">{item.category_code}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{item.category_name}</td>
                                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 hidden md:table-cell">{item.category_name_en || '-'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">{getStatusBadge(item.is_active)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div className="flex items-center justify-center gap-2">
                                            <button onClick={() => handleEdit(item.category_id)} className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors" title="แก้ไข"><Edit2 size={18} /></button>
                                            <button onClick={() => handleDelete(item.category_id)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="ลบ"><Trash2 size={18} /></button>
                                        </div>
                                    </td>
                                </tr>
                            )) : <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">ไม่พบข้อมูลหมวดสินค้า</td></tr>}
                        </tbody>
                    </table>
                </div>
                )}
            ...
            <ProductCategoryFormModal isOpen={isModalOpen} onClose={handleModalClose} editId={editingId} />
        </div>
    );
}
