/**
 * @file ItemTypeForm.tsx
 * @description หน้าฟอร์มจัดการข้อมูลประเภทสินค้า (Item Type Master Data)
 * @route /master-data/item-type
 */
...
export default function ItemTypeForm() {
    ...
    const handleNew = () => {
        ...
        setSaveError(null);
        navigate('/master-data/item-type');
    };

    const handleSave = async () => {
        if (!formData.itemTypeCode.trim() || !formData.itemTypeName.trim()) {
            setSaveError('กรุณากรอกรหัสและชื่อประเภทสินค้า');
            return;
        }
        setIsSaving(true);
        setSaveError(null);
    ...
    const handleFind = () => {
        ...
        if (found) {
            ...
            navigate(`/master-data/item-type?id=${found.item_type_id}`);
        } else {
            alert('ไม่พบรหัสประเภทสินค้าที่ค้นหา');
        }
    };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <div className="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-4 sm:gap-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 flex-1">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">รหัสประเภทสินค้า</label>
                        <input type="text" value={formData.itemTypeCode} readOnly className={`${styles.inputFlex} bg-gray-100 dark:bg-gray-600`} />
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 flex-1">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">ชื่อประเภทสินค้า</label>
                        <input type="text" value={formData.itemTypeName} readOnly className={`${styles.inputFlex} bg-gray-100 dark:bg-gray-600`} />
                    </div>
                </div>
                ...
                <div className="p-4 sm:p-6 space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 sm:w-32 shrink-0">รหัสประเภทสินค้า</label>
                            <div className="flex gap-2 min-w-0 flex-1">
                                <input type="text" value={formData.itemTypeCodeSearch} onChange={(e) => handleInputChange('itemTypeCodeSearch', e.target.value)} className={styles.inputFlex} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 sm:w-32 shrink-0">ชื่อประเภทสินค้า</label>
                            <input type="text" value={formData.itemTypeName} onChange={(e) => handleInputChange('itemTypeName', e.target.value)} className={styles.inputFlex} />
                        </div>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 sm:w-32 shrink-0">ชื่อประเภทสินค้า (EN)</label>
                            <input type="text" value={formData.itemTypeNameEn} onChange={(e) => handleInputChange('itemTypeNameEn', e.target.value)} className={styles.inputFlex} />
                        </div>
                    </div>
                </div>
