/**
 * @file IssueFormModal.tsx
 * @description หน้าต่างฟอร์มสร้าง/แก้ไข/ดูรายละเอียดใบเบิก (Stock Issue Form Modal)
 */
...
import { IssueFormLines } from './IssueFormLines';
...
export const IssueFormModal: React.FC<IssueFormModalProps> = ({
    ...
    const { data: conversionData, isLoading: isLoadingConversions } = useQuery({
        queryKey: ['issue-uom-conversions', activeUomItemId],
    ...
    const { data: barcodeData } = useQuery({
        queryKey: ['issue-item-barcodes', activeUomItemId],
    ...
    const onFormSubmit: SubmitHandler<IssueStockHeaderFormData> = (data) => {
        setPendingData(data);
    ...
    const formTitle = readOnly
        ? 'รายละเอียดใบเบิก (VIEW Stock Issue)'
        : isEditMode
            ? 'แก้ไขใบเบิก (EDIT Stock Issue)'
            : 'สร้างใบเบิกใหม่ (CREATE Stock Issue)';
        ...
        <>
            ...
            >
                <FormProvider {...formMethods}>
                    ...
                    <div className="flex-1 overflow-auto bg-slate-100 dark:bg-[#0b1120] p-6 space-y-6 animate-form-fade-in">
                        {isLoading ? (
                            ...
                            <form id="issue-stock-form" onSubmit={formMethods.handleSubmit(onFormSubmit, handleFormError)} className="w-full space-y-6">
                                {/* 1. Header Section */}
                ...
                <ConfirmationModal 
                    ...
                    description="คุณต้องการบันทึกข้อมูลใบเบิกนี้ใช่หรือไม่?"
