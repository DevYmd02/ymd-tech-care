...
export const TransferInFormLines: React.FC<Props> = ({ warehouses, uoms, icOptions }) => {
    const { control, watch } = useFormContext<TransferInFormValues>();
                ...
                <table className="w-full min-w-[2400px] border-collapse text-sm">
                    <thead className="sticky top-0 z-10">
                        <tr className="bg-emerald-600 text-white border-b border-emerald-500/30">
                            <th className="p-3 w-12 text-center font-bold border-r border-emerald-500/30 uppercase tracking-wider text-[11px]">ลำดับ</th>
                            <th className="p-3 w-48 text-left font-bold border-r border-emerald-500/30 uppercase tracking-wider text-[11px]">รหัสสินค้า (Item ID)</th>
                            ...
                            <th className="p-3 w-32 text-right font-bold border-r border-emerald-500/30 uppercase tracking-wider text-[11px]">จำนวน</th>
                            <th className="p-3 w-32 text-right font-bold border-r border-emerald-500/30 uppercase tracking-wider text-[11px]">อนุมัติแล้ว</th>
                            ...
                            <th className="p-3 w-40 text-left font-bold border-r border-emerald-500/30 uppercase tracking-wider text-[11px]">รหัสล็อต</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300">
                        {fields.length === 0 ? (
                            ...
                            fields.map((field: any, index) => {
                                ...
                                return (
                                    <tr key={field._id || index} className="hover:bg-emerald-50/30 dark:hover:bg-emerald-900/5 transition-colors group">
                                        ...
                                        <td className="p-2 border-r border-gray-100 dark:border-gray-800">
                                            <input type="text" readOnly className={`${inputClass} text-right font-bold text-emerald-600`} value={Number(field.req_qty || field.qty || 0).toLocaleString()} />
                                        </td>
                                        <td className="p-2 border-r border-gray-100 dark:border-gray-800">
                                            <input type="text" readOnly className={`${inputClass} text-right font-bold text-blue-600`} value={Number(field.qty_approved || 0).toLocaleString()} />
                                        </td>
                                        <td className="p-2 border-r border-gray-100 dark:border-gray-800">
