/**
 * @file useTransferOut.ts
 * @description React Hook Form สำหรับ Transfer Out
 */
...
import { transferOutFormSchema, type TransferOutFormValues } from '../schemas/transfer-out.schemas';
import { TransferOutService } from '../services/transfer-out.services';
...
const DEFAULT_VALUES: Partial<TransferOutFormValues> = {
    transfer_out_date: getTodayISO(),
    branch_id: '',
    emp_dept_id: '',
    status: 'COMPLETED',
    lines: [
...
interface UseTransferOutOptions {
    isOpen: boolean;
    onClose: () => void;
    editId?: string | null;
    pendingData?: import('../types/transfer-out.types').PendingTransferOutItem | null;
    onSuccess?: () => void;
}

export function useTransferOut({ isOpen, onClose, editId, pendingData, onSuccess }: UseTransferOutOptions) {
    const queryClient = useQueryClient();
    const { user } = useAuth();
    const isEditMode = !!editId;

    const formMethods = useForm<TransferOutFormValues>({
        resolver: zodResolver(transferOutFormSchema) as never,
        defaultValues: DEFAULT_VALUES as DefaultValues<TransferOutFormValues>,
        mode: 'onChange',
    });
    ...
    useEffect(() => {
        if (Object.keys(errors).length > 0) {
            console.error('[useTransferOut] Form Errors:', errors);
        }
    }, [errors]);
    ...
    const { data: editData, isLoading: isLoadingEdit } = useQuery({
        queryKey: ['transfer-out', editId],
        queryFn: () => (editId ? TransferOutService.getById(editId) : null),
        ...
        if (sourceData && isOpen) {
            const handleHydration = async () => {
                ...
                const rawLinesArray = (isEdit 
                    ? (sourceDataRecord.transferOutLines || sourceDataRecord.lines || [])
                ...
                if (allItemIds.length > 0) {
                    try {
                        ...
                        console.warn('[useTransferOut] items load failed:', err);
                    }
                    ...
                    try {
                        ...
                        console.warn('[useTransferOut] conversions load failed:', err);
                    }
                }
                ...
                try {
                    ...
                    console.warn('[useTransferOut] loc/lot load failed:', err);
                }

                reset({
                    ...DEFAULT_VALUES,
                    transfer_out_no: String(sourceDataRecord.transfer_out_no || ''),
                    transfer_out_date: String(sourceDataRecord.transfer_out_date || getTodayISO()),
                    ...
                    branch_id: sourceDataRecord.branch_id ? String(sourceDataRecord.branch_id) : (user?.employee?.branch_id ? String(user.employee.branch_id) : ''),
                ...
                } as unknown as TransferOutFormValues);
            };
    ...
    const createMutation = useMutation({
        mutationFn: (data: TransferOutFormValues) => TransferOutService.create(data as unknown as Parameters<typeof TransferOutService.create>[0]),
        onSuccess: (result: { success: boolean; message?: string }) => {
            if (result.success) {
                toast.success('บันทึกสำเร็จ');
                queryClient.invalidateQueries({ queryKey: ['transfer-out-list'] });
                onSuccess?.();
                onClose();
            } else {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: TransferOutFormValues }) => TransferOutService.update(id, data as unknown as Parameters<typeof TransferOutService.update>[1]),
        onSuccess: (result: { success: boolean; message?: string }) => {
            if (result.success) {
                toast.success('แก้ไขสำเร็จ');
                queryClient.invalidateQueries({ queryKey: ['transfer-out-list'] });
                queryClient.invalidateQueries({ queryKey: ['transfer-out', editId] });
                onSuccess?.();
                onClose();
            } else {
                toast.error(result.message || 'เกิดข้อผิดพลาด');
    ...
    const { icOptions } = useICOptions(watchedBranchId, SYSTEM_DOCUMENT_CODES.INVENTORY_TRANSFER_OUT);
    ...
    const onSubmit = useCallback(async (data: TransferOutFormValues) => {
        try {
            const payload = {
                ...data,
