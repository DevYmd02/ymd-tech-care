...
interface TransferOutFormHeaderProps {
    readOnly?: boolean;
    pendingData?: any;
    branches?: any[];
    employees?: any[];
...
export function TransferOutFormHeader({ readOnly = false, pendingData, branches, employees, departments, projects = [] }: TransferOutFormHeaderProps) {
    const { register, control, watch } = useFormContext<TransferOutFormValues>();
    ...
    const getInputClass = (disabled: boolean) => 
        disabled 
            ...
            : "h-9 w-full px-3 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 text-gray-900 dark:text-white placeholder-gray-400 transition-all shadow-sm";

    const labelClass = "block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider";
    const cardSection = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-5 gap-y-4 bg-amber-50/10 dark:bg-amber-900/5 p-6 rounded-2xl border border-amber-100 dark:border-amber-900/20";
        ...
        <section className="space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                    ...
                    <h3 className="text-lg font-bold">ข้อมูลใบโอนย้ายสินค้าออก — Header Transfer Out</h3>
                </div>
            </div>
            ...
            <div className={cardSection}>
                ...
                <div className="space-y-1">
                    ...
                    <input
                        value={watch('transfer_out_no') || 'รอสร้างเอกสาร...'}
                        disabled={true}
                        readOnly
                        className={getInputClass(true)}
                    />
                    <input type="hidden" {...register('transfer_out_no')} />
                </div>
