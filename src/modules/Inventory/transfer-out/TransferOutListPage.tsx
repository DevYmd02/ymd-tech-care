/**
 * @file TransferOutListPage.tsx
 * @description หน้ารายการใบโอนย้ายสินค้าออก (Transfer Out List)
 * @route /inventory/transfer-out
 */

import { useState, useMemo, useCallback } from 'react';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { ArrowUpRight, Plus, Search, Eye, Edit, X } from 'lucide-react';
...
import type { TransferOutListItem, TransferOutListParams, PendingTransferOutItem } from './types/transfer-out.types';

const colHelper = createColumnHelper<TransferOutListItem>();
const pendingColHelper = createColumnHelper<PendingTransferOutItem>();

export default function TransferOutListPage() {
    ...
    const [pendingDataForModal, setPendingDataForModal] = useState<PendingTransferOutItem | null>(null);

    const apiParams: TransferOutListParams = {
        transfer_out_no: filters.search || undefined,
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['transfer-out-list', apiParams],
        queryFn: ({ signal }) => TransferOutService.getList(apiParams, { signal }),
        staleTime: 0,
        enabled: activeTab === 'history',
    });

    const { data: pendingData, isLoading: isPendingLoading } = useQuery({
        queryKey: ['transfer-out-pending'],
        queryFn: ({ signal }) => TransferOutService.getPendingList(undefined, { signal }),
        staleTime: 0,
    ...
    const columns = useMemo(
        () => [
            ...
            colHelper.accessor('transfer_out_no', {
                header: 'เลขที่เอกสาร',
                cell: info => (
                    <span
                        className="font-semibold text-blue-600 hover:underline cursor-pointer"
                        onClick={() => handleView(info.row.original.transfer_out_id)}
                    >
                        {info.getValue()}
                    </span>
                ),
            ...
            colHelper.display({
                ...
                cell: ({ row }) => (
                    <div className="flex justify-center gap-2">
                        <button onClick={() => handleView(row.original.transfer_out_id)} className="text-gray-500 hover:text-blue-600"><Eye size={16} /></button>
                        <button onClick={() => handleEdit(row.original.transfer_out_id)} className="text-gray-500 hover:text-amber-600"><Edit size={16} /></button>
                    </div>
                ),
            }),
        ],
    ...
    return (
        <PageListLayout
            title="ใบโอนย้ายสินค้าออก (Transfer Out)"
            subtitle="จัดการข้อมูลการโอนย้ายสินค้าและวัตถุดิบออกจากคลังสินค้า"
            icon={ArrowUpRight}
            accentColor="amber"
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div className="md:col-span-4 flex flex-col md:flex-row md:justify-end items-center gap-3 mt-2">
                        ...
                        <button
                            ...
                            className="h-10 w-full md:w-auto bg-amber-600 hover:bg-amber-700 text-white rounded-lg font-bold shadow-sm transition-all active:scale-95 flex items-center justify-center px-6 gap-2"
                        >
        ...
        >
            <div className="flex border-b border-gray-200 dark:border-gray-700 mb-4">
                ...
                >
                    รายการรอโอนย้ายออก {pendingData ? `(${pendingData.length})` : ''}
                </button>
                <button
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                {activeTab === 'pending' ? (
                    <SmartTable
                        data={pendingData ?? []}
                        columns={pendingColumns as ColumnDef<PendingTransferOutItem, unknown>[]}
                    ...
                    <SmartTable
                        data={data?.items ?? []}
                        columns={columns as ColumnDef<TransferOutListItem, unknown>[]}
                        isLoading={isLoading}
                        pagination={{
            ...
            {isFormOpen && (
                <TransferOutModal
                    ...
                    onSuccess={() => {
                        ...
                        queryClient.invalidateQueries({ queryKey: ['transfer-out-pending'] });
                    }}
                />
            )}
        </PageListLayout>
