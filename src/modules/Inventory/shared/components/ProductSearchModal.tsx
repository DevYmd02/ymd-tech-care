 ...
 * @description Localized Search Modal for selecting Products in Inventory module.
 * @pattern ตามสไตล์ของ Sales Module เพื่อความเป็นระเบียบและสวยงามระดับ Premium
 */
...
export const ProductSearchModal: React.FC<ProductSearchModalProps> = React.memo(({
    ...
    // ดึงข้อมูลสินค้าผ่าน ItemMasterService (ใช้วิธี Server-side search เพื่อประสิทธิภาพ)
    const { data: response, isLoading } = useQuery({
        queryKey: ['inventory-products-lookup', debouncedSearch],
        queryFn: () => ItemMasterService.getAll({ q: debouncedSearch, limit: 100 }),
        enabled: isOpen,
        staleTime: 1000 * 60 * 5, // Cache ข้อมูลไว้นานหน่อยเพื่อลดการโหลดซ้ำ
    });
            ...
            <div className="flex flex-col h-[70vh]">
                ...
                <div className="flex-1 overflow-auto p-0">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-24 opacity-60">
                            ...
                            <p className="text-gray-500 font-medium font-outfit">กำลังโหลดข้อมูลสินค้า...</p>
                        </div>
                    ) : (
                                ...
                                {products.length > 0 ? (
                                    products.map((product) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-semibold text-gray-800 dark:text-gray-200 line-clamp-2 leading-relaxed">
                                                    {product.item_name}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center text-gray-400 dark:text-gray-500">
                                                ...
                                                <p className="text-xl font-bold font-outfit">ไม่พบข้อมูลสินค้า</p>
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800/80 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center px-6">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium font-outfit">
                        แสดงข้อมูล <span className="font-bold text-blue-600">{products.length}</span> รายการ
                    </p>
                    <button 
                        onClick={onClose}
