/**
 * @file TransferSearchModal.tsx
 * @description หน้าต่างค้นหาใบขอโอนย้ายสินค้าที่ค้างอนุมัติ (Pending Transfer Requisitions Search Modal)
 */
...
export interface TransferSearchModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSelect: (transferId: string, item?: TransferRequisitionListItem) => void;
}

export const TransferSearchModal: React.FC<TransferSearchModalProps> = React.memo(({
    isOpen,
    onClose,
    onSelect,
}) => {
    ...
    // Fetch pending Transfer Requisitions
    const { data: rawData = [], isLoading } = useQuery({
        queryKey: ['transfer-requisitions-pending', isOpen],
    ...
    return (
        <DialogFormLayout
            isOpen={isOpen}
            onClose={onClose}
            title="ค้นหาใบขอโอนย้ายที่รออนุมัติ - Find Pending Transfer Requisition"
            titleIcon={
                ...
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            placeholder="ค้นหาเลขที่เอกสาร, สาขา, ผู้ขอโอน..."
                        ...
                        <table className="w-full text-left border-separate border-spacing-0">
                            <thead className="sticky top-0 z-10 bg-gray-50/90 dark:bg-gray-800/90 backdrop-blur-md">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-xs font-extrabold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">สาขา</th>
                                    <th className="px-6 py-4 text-xs font-extrabold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">ผู้ขอโอน/ผู้บันทึก</th>
...
TransferSearchModal.displayName = 'TransferSearchModal';
