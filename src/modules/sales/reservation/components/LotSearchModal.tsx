...
export const LotSearchModal: React.FC<LotSearchModalProps> = React.memo(({
    ...
    // Fetch lots - if itemId is provided, filtering happens backend-side
    const { data: response, isLoading } = useQuery({
        queryKey: ['lot-lookup', debouncedSearch, itemId],
        queryFn: () => LotNoService.getAll({ q: debouncedSearch, item_id: itemId, limit: 100 }),
        enabled: isOpen,
    });
                        ...
                        <table className="w-full text-left border-separate border-spacing-0">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-[#0b1120]/30 transition-all">
                                {lots.length > 0 ? (
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-blue-600 dark:text-blue-400 group-hover:scale-105 transition-transform inline-block w-fit">
                                                        {lot.code}
                                                    </span>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-6 py-16 text-center items-center justify-center">
