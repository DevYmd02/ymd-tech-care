...
export const ReservationLineSchema = z.object({
    ...
    location_id: z.string().min(1, 'กรุณาเลือกคลังย่อย'),
    uom_id: z.coerce.string().min(1, 'กรุณาเลือกหน่วยนับ'),
    unit_price: z.coerce.number().min(0, 'ราคาต่อหน่วยต้องไม่ติดลบ'),
    lot_id: z.coerce.number().min(1, 'กรุณาเลือกล็อตสินค้า'),
    lot_no: z.string().min(1, 'กรุณาเลือกล็อตสินค้า'),
    lot_balance_id: z.coerce.number().min(1, 'กรุณาเลือกล็อตสินค้าที่มียอดคงเหลือ'),
...
export const ReservationFormSchema = z.object({
    ...
    so_id: z.coerce.string().nullable().optional().transform(v => v === '' ? null : v),
    ...
    status: z.enum(['DRAFT', 'CONFIRMED', 'POSTED', 'RELEASED', 'EXPIRED', 'CANCELLED']).default('DRAFT'),
