                ...
                <table className="w-full min-w-[1400px] border-collapse bg-white dark:bg-gray-900 text-sm">
                    <thead className="bg-blue-600 text-white text-xs">
                        <tr>
                            ...
                            <th className="p-2 w-24 text-center border-r border-blue-500 bg-blue-700">{showRequestedQty ? 'จำนวนที่ขอ' : 'จำนวนที่เหลือ'}</th>
                    ...
                    <tbody>
                        {(() => {
                            ...
                            let currentLineDiscount = 0;
                            const currentDiscountInput = String(line.line_discount_raw || '').trim();
                            const linePrice = Number(line.est_unit_price || 0);
                            
                            if (currentDiscountInput.endsWith('%')) {
                                const percent = parseFloat(currentDiscountInput.replace('%', ''));
                                if (!isNaN(percent)) {
                                    currentLineDiscount = (approvedQty * linePrice) * (percent / 100);
                                }
                            } else {
                                currentLineDiscount = parseFloat(currentDiscountInput) || 0;
                            }
                            
                            const lineTotal = isApproved ? Math.max(0, (approvedQty * linePrice) - currentLineDiscount) : 0;
                            ...
                            return (
                                <tr key={index} className={`border-b border-gray-200 dark:border-gray-700 transition-colors ${!isApproved ? 'bg-gray-100/50 dark:bg-gray-800/50 opacity-60' : 'hover:bg-blue-50 dark:hover:bg-gray-800'}`}>
                                    <td className="p-1 text-center border-r border-gray-300 dark:border-gray-600 sticky left-0 z-10 bg-slate-100 dark:bg-slate-800">
                                        <input
                                          type="checkbox"
                                          disabled={isExistingAV || readOnly}
                                          checked={isApproved}
                                          onChange={(e) => updateLine(index, 'is_approved', e.target.checked)}
                                          className="w-4 h-4 text-blue-600 rounded cursor-pointer"
                                        />
                                    </td>
                                    <td className="p-1 text-center bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold border-r border-gray-300 dark:border-gray-600 sticky left-[40px] z-10">{visibleNo}</td>
                                    ...
                                    <td className={`${tdBaseClass} bg-green-50 dark:bg-green-900/10`}>
                                        <div className="relative">
                                            <input 
                                                ...
                                                disabled={!isApproved || isExistingAV || readOnly}
                                    ...
                                    <td className={tdBaseClass}>
                                        <div className="px-2 h-8 flex items-center justify-end text-gray-700 dark:text-gray-300 text-sm">
                                            {currentLineDiscount ? currentLineDiscount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}
                                        </div>
                                    </td>
