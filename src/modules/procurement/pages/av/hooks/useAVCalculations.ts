...
interface AVCalculationsResult {
  subtotal: number;
...
/**
 * Hook for AV Totals Calculations
 * Calculates only lines where `is_approved` is true, using `approved_qty`
 */
export const useAVCalculations = (props?: UseAVCalculationsProps): AVCalculationsResult => {
  const context = useFormContext<AVFormData>();
  ...
  // 1. Calculate Line-Level Totals (only for approved lines)
  const { subtotal, totalGross, totalLineDiscount } = useMemo(() => {
    return (lines || []).reduce((acc: { subtotal: number, totalGross: number, totalLineDiscount: number }, line: AVLineFormData) => {
      // Skip if not approved
      if (!line.is_approved) return acc;

      const qty = Number(line?.approved_qty ?? line?.qty);
      ...
      // Calculate ratio of discount if qty was changed.
      // If original qty was 10, discount was 100, and approved line is 5, discount should probably be scaled or kept 0?
      // Usually, PR line discount is per unit or total. Let's assume it scales or just use what's there but maybe user fixes it.
      // For simplicity, we just use the raw line discount if user hasn't scaled it, but we can do a ratio.
      const originalQty = Number(line?.qty) || 1;
      const lineDiscountRaw = Number(line?.discount);
      const safeLineDiscount = isNaN(lineDiscountRaw) ? 0 : (lineDiscountRaw * (safeQty / originalQty));
