...
import { POFormSchema, type POFormData } from '@/modules/procurement/schemas/po-schemas';
...
export const usePOForm = ({
    ...
    const products: ItemListItem[] = [
        { id: 1, item_id: 1, item_code: 'ITEM-001', item_name: 'กระดาษ A4', description: 'กระดาษ A4 80 แกรม', unit_name: 'REAM', unit_id: 1, uom_id: 1, standard_cost: 100, category_id: 1, category_name: 'Office Supplies', is_active: true, created_at: new Date().toISOString() },
        { id: 2, item_id: 2, item_code: 'ITEM-002', item_name: 'ปากกาน้ำเงิน', description: 'ปากกาลูกลื่น 0.5mm', unit_name: 'PCS', unit_id: 2, uom_id: 2, standard_cost: 3, category_id: 1, category_name: 'Office Supplies', is_active: true, created_at: new Date().toISOString() },
    ];
    ...
    const handleSelectProduct = useCallback((product: ItemListItem) => {
        if (activeRowIndex !== null) {
            setValue(`lines.${activeRowIndex}.item_id`, Number(product.item_id));
            ...
            setValue(`lines.${activeRowIndex}.uom_id`, Number(product.uom_id || product.unit_id) || 1);
