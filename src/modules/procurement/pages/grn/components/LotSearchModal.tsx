...
export const LotSearchModal: React.FC<LotSearchModalProps> = React.memo(({
    ...
    warehouseId,
    locationId,
    title = 'จัดการล็อตและสต็อกสินค้า (GRN)'
}) => {
    ...
    // Enrich balances with master lot data
    const availableLots = useMemo(() => {
        ...
        return balances.map(balance => {
            ...
            return {
                ...balance,
                id: balance.lot_balance_id || balance.id, 
                lot_id: masterId, 
        ...
        <>
                    ...
                    <div className="p-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 space-y-4">
                        ...
                        {(warehouseId || locationId) && activeTab === 'available' && (
                            <div className={`mb-4 px-4 py-2.5 rounded-xl border flex items-center gap-3 transition-all duration-300 ${
                                ...
                                <div className={`p-1.5 rounded-lg ${ignoreFilters ? 'bg-gray-200 dark:bg-gray-700 text-gray-500' : 'bg-indigo-500 text-white shadow-sm shadow-indigo-500/20'}`}>
                                    <MapPin size={14} />
                                </div>
                                <div className="flex-1">
                                    <p className={`text-[11px] font-bold uppercase tracking-wider ${ignoreFilters ? 'text-gray-400' : 'text-indigo-500'}`}>
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 ${
                                        ignoreFilters
                                            ...
                                            : 'bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50'
                                    }`}
                                >
                        ...
                        <div className="relative group max-w-md">
                            ...
                            <input
                                ...
                                className="w-full h-11 pl-11 pr-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-medium shadow-sm transition-all text-gray-900 dark:text-white"
                            />
                        </div>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-800/80 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center px-6">
                        <div className="flex gap-6 text-sm">
                            <p className="text-gray-500 dark:text-gray-400">
                                พบทั้งหมด <span className="font-bold text-indigo-600">{activeTab === 'available' ? availableLots.length : masterLots.length}</span> รายการ
                            </p>
                            {activeTab === 'available' && (
                                <p className="text-gray-500 dark:text-gray-400">
            ...
            {selectedLotForAdjust && (
                <QuickAdjustStockModal
                    ...
                    lotId={selectedLotForAdjust.id} 
                    lotNoId={selectedLotForAdjust.lot_id || 0} 
            ...
            {isCreateBalanceOpen && (
                <CreateBalanceModal
                    ...
                    onCreated={() => setActiveTab('available')} 
                />
            )}
        </>
    );
