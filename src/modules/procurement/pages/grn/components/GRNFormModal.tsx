...
export default function GRNFormModal({ isOpen, onClose, id, onSuccess, initialPOId, readOnly = false }: Props) {
    const {
        methods,
        isSubmitting,
        isFetchingPO,
        ...
        remove,
        onClose: handleClose
    } = useGRNForm({ isOpen, id, initialPOId, onClose, onSuccess, readOnly });
    ...
    return (
        <FormProvider {...methods}>
            <WindowFormLayout
                isOpen={isOpen}
                onClose={handleClose}
                title={readOnly ? 'รายละเอียดใบรับสินค้า (GRN Detail)' : id ? 'แก้ไขใบรับสินค้า (Edit GRN)' : 'สร้างใบรับสินค้าใหม่ (Create GRN)'}
            ...
            >
                <ErrorBoundary>
                    {isLoadingDetail && !watch('grn_no') ? (
                         <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-950/50">
                            <Loader2 size={40} className="text-violet-500 animate-spin mb-4" />
                            <p className="text-gray-500 font-medium">กำลังโหลดข้อมูล...</p>
                        </div>
                    ) : (
                        <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-950 p-6 gap-6 overflow-auto animate-form-fade-in relative">
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <div className="relative h-8">
                                                    <CustomDateInput value={field.value} onChange={field.onChange} className={inputClass} />
                                                </div>
                                            )}
                                        />
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="px-4 h-8 bg-violet-600 text-white rounded-lg hover:bg-violet-700 shadow-sm transition-all flex items-center justify-center"
                                            >
                                            ...
                                            >
                                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="relative">
                                                            <select {...register('curr_id')} className={selectClass} disabled={!isMulticurrency}>
                                                    ...
                                                    <div>
                                                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">ไปยังสกุลเงิน</label>
                                                        <div className="relative">
                                                            <select {...register('curr_type_id')} className={selectClass} disabled={!isMulticurrency}>
                                                                <option value="">-- เลือกสกุลเงิน --</option>
                                                                {currencies.map((c: Currency) => (
                                                                    <option key={c.id || c.currency_id} value={c.id || c.currency_id}>
                                                                        {c.code || c.currency_code} - {c.name_th}
                                            ...
                                            {fields.map((item, index) => (
                                                <tr key={item.id} className="group hover:bg-violet-50/30 dark:hover:bg-violet-900/10 transition-colors">
                                                    ...
                                                    <td className="px-6 py-3">
                                                        <input type="text" {...register(`items.${index}.item_code`)} readOnly className="w-full h-9 px-2 text-sm font-bold text-gray-700 dark:text-gray-100 border border-transparent rounded bg-transparent outline-none" />
                                                    </td>
                                                    <td className="px-6 py-3">
                                                        <input type="text" {...register(`items.${index}.item_name`)} readOnly className="w-full h-9 px-2 text-sm font-medium text-gray-600 dark:text-gray-300 border border-transparent rounded bg-transparent outline-none" />
                                                    </td>
                                                    <td className="px-6 py-3">
                                                        <input type="number" {...register(`items.${index}.qty_ordered`)} readOnly className="w-full h-9 px-2 text-sm text-center border border-gray-100 dark:border-gray-800 rounded bg-gray-50/50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-400 font-medium" />
                                                    </td>
                                                    <td className="px-6 py-3">
                                                        <Controller
                                                            control={control}
                                                    ...
                                                    <td className="px-6 py-3">
                                                        <input type="text" {...register(`items.${index}.uom_name`)} readOnly className="w-full h-9 px-2 text-sm text-gray-700 dark:text-gray-300 border border-transparent rounded bg-transparent outline-none" />
                                                    </td>
                                                    <td className="px-6 py-3">
                                                    ...
                                                    <td className="px-6 py-3">
                                                        <input type="text" {...register(`items.${index}.remark`)} placeholder="หมายเหตุ" className="w-full h-9 px-2 text-sm text-gray-700 dark:text-gray-300 border border-transparent group-hover:border-gray-200 dark:group-hover:border-gray-700 rounded bg-transparent focus:bg-white dark:focus:bg-gray-800 transition-all outline-none" />
                                                    </td>
                                                    <td className="px-4 py-3 text-center">
                                                        <button type="button" onClick={() => remove(index)} className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-all">
                                                            <Trash2 size={18} />
                                                        </button>
                                                    </td>
                                                </tr>
