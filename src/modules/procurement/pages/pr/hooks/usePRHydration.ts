 ...
 * Encapsulates complex mapping logic to keep usePRForm clean.
 */
export const usePRHydration = ({ 
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // 🛑 Abort any previous hydration request for this specific hook instance
      ...
      setIsLoading(true);
      try {
        logger.info(`[usePRHydration] Starting hydration for ID: ${id}`);
        ...
        if (pr && isActive) {
          ...
          const locationMaps = await Promise.all(
            uniqueWhIds.map(async (whId) => {
              try {
                ...
                if (!signal.aborted) logger.error(`[usePRHydration] Failed to fetch locations for warehouse ${whId}:`, err);
          ...
          if (!vendorName && vendorId) {
            try {
              ...
              if (!signal.aborted) logger.error('[usePRHydration] Vendor lookup failed:', err);
            }
          }
          ...
          hydrationIdRef.current = id;
          onDataLoaded(formData);
          logger.info('[usePRHydration] Success');
        }
      } catch (error: unknown) {
        if (error instanceof Error && (error.name === 'AbortError' || error.name === 'CanceledError')) {
          logger.debug('[usePRHydration] Aborted');
          return;
        }
        logger.error('[usePRHydration] Error:', error);
      } finally {
        if (isActive) {
          setIsLoading(false);
        }
    ...
    if (isOpen && id && !isMasterDataLoading && warehouses.length > 0) {
      // Logic: Only hydration if ID changed OR we don't have a hydrated ID yet
  ...
  }, [isOpen, id, isMasterDataLoading, warehouses, masterItems, masterUnits, onDataLoaded]);
