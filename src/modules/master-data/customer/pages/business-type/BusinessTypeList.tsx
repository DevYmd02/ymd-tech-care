/**
 * @file BusinessTypeListPage.tsx
 * @description หน้าจัดการประเภทธุรกิจลูกค้า (Customer Business Type)
 */
...
export default function BusinessTypeList() {
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('business_type_code', {
      header: 'รหัส',
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex justify-center gap-2">
          <button onClick={() => handleEdit(row.original.business_type_id)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg">
            <Edit2 size={18} />
          </button>
          <button onClick={() => handleDelete(row.original.business_type_id, row.original.business_type_code)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg">
            <Trash2 size={18} />
          </button>
        </div>
      ),
    }),
  ], [filters.page, filters.limit, handleDelete, columnHelper]);

  return (
    <PageListLayout
      title="กำหนดประเภทธุรกิจ"
      subtitle="Customer Business Type Setup"
      icon={Briefcase}
      accentColor="blue"
