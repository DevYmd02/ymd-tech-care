...
export default function ShelfList() {
    ...
    const [editingData, setEditingData] = useState<Shelf | null>(null);
    ...
    const { data: response, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (sortConfig) {
                items.sort((a, b) => {
                    const fieldValA = a[sortConfig.key as keyof Shelf];
                    const fieldValB = b[sortConfig.key as keyof Shelf];
    ...
    const handleEdit = useCallback((item: Shelf) => {
        ...
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                    <LayoutGrid className="text-blue-600" /> กำหนดชั้นวางสินค้า (Shelf Master)
                </h1>
                <p className="text-gray-500 mt-1 text-sm">จัดการข้อมูลชั้นวางสินค้า</p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <FilterFormBuilder config={filterConfig} filters={filters} onFilterChange={(name: string, value: string) => setFilters({ [name]: value })} onSearch={() => handlePageChange(1)} onReset={resetFilters} onCreate={handleCreateNew} createLabel="เพิ่มชั้นวางใหม่" accentColor="indigo" />
            </div>
