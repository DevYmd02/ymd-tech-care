/**
 * @file ShelfList.tsx - หน้ารายการชั้นวางสินค้า
 */
...
export default function ShelfList() {
    ...
    const [allData, setAllData] = useState<Shelf[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingId, setEditingId] = useState<string | null>(null);

    const filterConfig: FilterFieldConfig<keyof typeof filters>[] = useMemo(() => [
        { name: 'search', label: 'รหัสชั้นวาง', type: 'text', placeholder: 'กรอกรหัส' },
        { name: 'search2', label: 'ชื่อชั้นวาง', type: 'text', placeholder: 'กรอกชื่อ' },
        { name: 'status', label: 'สถานะ', type: 'select', options: STATUS_OPTIONS },
    ], []);

    const fetchData = useCallback(async () => { setIsLoading(true); try { const response = await ShelfService.getAll(); setAllData(response.items); } finally { setIsLoading(false); } }, []);
    ...
    const handleDelete = useCallback((id: string) => { if (confirm('ลบข้อมูลนี้?')) ShelfService.delete(id).then(() => fetchData()); }, [fetchData]);
    ...
    return (
        <div className="p-6 space-y-6">
            <div><h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><LayoutGrid className="text-blue-600" /> กำหนดชั้นวางสินค้า (Shelf Master)</h1><p className="text-gray-500 mt-1 text-sm">จัดการข้อมูลชั้นวางสินค้า</p></div>
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700"><FilterFormBuilder config={filterConfig} filters={filters} onFilterChange={(name: string, value: string) => setFilters({ [name]: value })} onSearch={() => handlePageChange(1)} onReset={resetFilters} onCreate={() => { setEditingId(null); setIsModalOpen(true); }} createLabel="เพิ่มชั้นวางใหม่" accentColor="indigo" /></div>
            <div className="flex flex-col gap-4"><h2 className="text-gray-700 font-medium">พบข้อมูล {filteredData.length} รายการ</h2><SmartTable data={paginatedData} columns={columns} isLoading={isLoading} pagination={{ pageIndex: filters.page, pageSize: filters.limit, totalCount: filteredData.length, onPageChange: handlePageChange, onPageSizeChange: (size) => setFilters({ limit: size, page: 1 }) }} rowIdField="shelf_id" /></div>
            <ShelfFormModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingId(null); }} editId={editingId} onSuccess={fetchData} />
        </div>
    );
}
