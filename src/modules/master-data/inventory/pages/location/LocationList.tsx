/**
 * @file LocationList.tsx - หน้ารายการที่เก็บสินค้า
 */
import { useState, useEffect, useCallback, useMemo } from 'react';
import { Edit2, Trash2, MapPin } from 'lucide-react';
import { LocationFormModal } from './LocationFormModal';
import { LocationService } from '../../services/inventory-master.service';
import type { Location } from '../../types/inventory-master.types';
...
export default function LocationList() {
    ...
    const filterConfig: FilterFieldConfig<keyof typeof filters>[] = useMemo(() => [
        ...
        { name: 'search2', label: 'ชื่อที่เก็บ', type: 'text', placeholder: 'กรอกชื่อ' },
        { name: 'status', label: 'สถานะ', type: 'select', options: STATUS_OPTIONS },
    ], []);

    const fetchData = useCallback(async () => { setIsLoading(true); try { const response = await LocationService.getAll(); setAllData(response.items); } finally { setIsLoading(false); } }, []);
