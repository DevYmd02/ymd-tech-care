...
export default function ColorList() {
    ...
    const [editingData, setEditingData] = useState<Color | null>(null);
    ...
    const { data: response, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (sortConfig) {
                items.sort((a, b) => {
                    const fieldValA = a[sortConfig.key as keyof Color];
                    const fieldValB = b[sortConfig.key as keyof Color];
    ...
    const handleEdit = useCallback((item: Color) => {
        ...
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                    <Palette className="text-blue-600" /> กำหนดสีสินค้า (Color Master)
                </h1>
                <p className="text-gray-500 mt-1 text-sm">จัดการข้อมูลสีสินค้า</p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <FilterFormBuilder config={filterConfig} filters={filters} onFilterChange={(name: string, value: string) => setFilters({ [name]: value })} onSearch={() => handlePageChange(1)} onReset={resetFilters} onCreate={handleCreateNew} createLabel="เพิ่มสีใหม่" accentColor="indigo" />
            </div>
            ...
            <ColorFormModal isOpen={isModalOpen} onClose={handleModalClose} editId={editingId} initialData={editingData} onSuccess={() => { refetch(); handleModalClose(); }} />
        </div>
    );
}
