...
export default function ItemTypeList() {
    ...
    const { data: response, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (filters.search) {
                ...
                items = items.filter(i => i.item_type_code.toLowerCase().includes(term));
            }
            if (filters.search2) {
                const term = filters.search2.toLowerCase();
                items = items.filter(i => i.item_type_name.toLowerCase().includes(term));
            }
    ...
    // ==================== TABLE COLUMNS ====================
    const columns = useMemo<ColumnDef<ItemTypeListItem>[]>(() => [
        {
            id: 'sequence',
            header: 'ลำดับ',
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        ...
                        กำหนดรหัสประเภทสินค้า (Item Type)
                    </h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
                        จัดการข้อมูลประเภทสินค้าทั้งหมด
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <FilterFormBuilder
                    ...
                    createLabel="เพิ่มประเภทสินค้าใหม่"
                    accentColor="indigo"
