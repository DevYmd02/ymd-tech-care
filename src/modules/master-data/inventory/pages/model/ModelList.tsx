...
export default function ModelList() {
    ...
    const [editingData, setEditingData] = useState<Model | null>(null);
    ...
    const { data: response, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (sortConfig) {
                items.sort((a, b) => {
                    const fieldValA = a[sortConfig.key as keyof Model];
                    const fieldValB = b[sortConfig.key as keyof Model];
    ...
    const handleEdit = useCallback((item: Model) => {
    ...
    const columns = useMemo<ColumnDef<Model>[]>(() => [
        { id: 'sequence', header: 'ลำดับ', accessorFn: (_, index) => (filters.page - 1) * filters.limit + index + 1, size: 60 },
        {
            ...
            header: 'รหัส',
            cell: ({ row }) => (
                <span className="font-medium text-blue-600 cursor-pointer hover:underline" onClick={() => handleEdit(row.original)}>
        ...
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                    <Box className="text-blue-600" /> กำหนดรุ่นสินค้า (Model Master)
                </h1>
                <p className="text-gray-500 mt-1 text-sm">จัดการข้อมูลรุ่นสินค้า</p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <FilterFormBuilder config={filterConfig} filters={filters} onFilterChange={(name: string, value: string) => setFilters({ [name]: value })} onSearch={() => handlePageChange(1)} onReset={resetFilters} onCreate={handleCreateNew} createLabel="เพิ่มรุ่นใหม่" accentColor="indigo" />
            </div>
            <div className="flex flex-col gap-4">
                <h2 className="text-gray-700 font-medium">พบข้อมูล {response?.total || 0} รายการ</h2>
                <SmartTable
                    data={response?.items || []}
                    columns={columns}
                    isLoading={isLoading}
