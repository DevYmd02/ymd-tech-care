/**
 * @file LotNoList.tsx - หน้ารายการ Lot No สินค้า
 */
import { useState, useEffect, useCallback, useMemo } from 'react';
import { Edit2, Trash2, Hash } from 'lucide-react';
import { LotNoFormModal } from './LotNoFormModal';
import { LotNoService } from '../../services/inventory-master.service';
import type { LotNo } from '../../types/inventory-master.types';
...
export default function LotNoList() {
    ...
    const handleDelete = useCallback((id: string) => { if (confirm('ลบข้อมูลนี้?')) LotNoService.delete(id).then(() => fetchData()); }, [fetchData]);

    const columns = useMemo<ColumnDef<LotNo>[]>(() => [
    ...
    return (
        <div className="p-6 space-y-6">
            <div><h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><Hash className="text-blue-600" /> กำหนด Lot No สินค้า (Lot No Master)</h1><p className="text-gray-500 mt-1 text-sm">จัดการข้อมูล Lot No สินค้า</p></div>
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700"><FilterFormBuilder config={filterConfig} filters={filters} onFilterChange={(name, value) => setFilters({ [name]: value })} onSearch={() => handlePageChange(1)} onReset={resetFilters} onCreate={() => { setEditingId(null); setIsModalOpen(true); }} createLabel="เพิ่ม Lot No ใหม่" accentColor="indigo" /></div>
            <div className="flex flex-col gap-4"><h2 className="text-gray-700 font-medium">พบข้อมูล {filteredData.length} รายการ</h2><SmartTable data={paginatedData} columns={columns} isLoading={isLoading} pagination={{ pageIndex: filters.page, pageSize: filters.limit, totalCount: filteredData.length, onPageChange: handlePageChange, onPageSizeChange: (size) => setFilters({ limit: size, page: 1 }) }} rowIdField="lot_no_id" /></div>
            <LotNoFormModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingId(null); }} editId={editingId} onSuccess={fetchData} />
        </div>
    );
}
