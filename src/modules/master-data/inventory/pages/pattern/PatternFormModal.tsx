 ...
 */
import { Grid3X3, Save, X, RotateCcw } from 'lucide-react';
...
export function PatternFormModal({ isOpen, onClose, editId, initialData, onSuccess }: Props) {
    const {
        register,
        errors,
        isSaving,
        handleSave,
        clearForm
    } = usePatternForm(editId ?? null, initialData, onSuccess);
    ...
    const TitleIcon = <Grid3X3 size={24} className="text-white" />;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-end pb-2">
                        <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700 w-full">
                            <input {...register('isActive')} type="checkbox" id="pattern_is_active" className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer" />
                            <label htmlFor="pattern_is_active" className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">สถานะใช้งาน</label>
                        </div>
                    </div>
                </div>
                <div>
