 ...
 * @description หน้ารายการรูปแบบสินค้า (Pattern Master Data List)
 */

import { useState, useEffect, useCallback, useMemo } from 'react';
import { Edit2, Trash2, Grid3X3 } from 'lucide-react';
...
export default function PatternList() {
    ...
    const filterConfig: FilterFieldConfig<keyof typeof filters>[] = useMemo(() => [
        ...
        { name: 'search2', label: 'ชื่อรูปแบบ', type: 'text', placeholder: 'กรอกชื่อ' },
        { name: 'status', label: 'สถานะ', type: 'select', options: STATUS_OPTIONS },
    ], []);

    const fetchData = useCallback(async () => { setIsLoading(true); try { const response = await PatternService.getAll(); setAllData(response.items); } finally { setIsLoading(false); } }, []);
    ...
    const columns = useMemo<ColumnDef<Pattern>[]>(() => [
        ...
        { id: 'actions', header: 'จัดการ', size: 100, cell: ({ row }) => (<div className="flex items-center gap-2"><button onClick={() => handleEdit(row.original.id)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={18} /></button><button onClick={() => handleDelete(row.original.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={18} /></button></div>)},
    ], [filters.page, filters.limit, handleDelete]);

    return (
        <div className="p-6 space-y-6">
            <div><h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><Grid3X3 className="text-blue-600" /> กำหนดรูปแบบสินค้า (Pattern Master)</h1><p className="text-gray-500 mt-1 text-sm">จัดการข้อมูลรูปแบบสินค้าในระบบ</p></div>
            ...
            <div className="flex flex-col gap-4"><h2 className="text-gray-700 font-medium">พบข้อมูล {filteredData.length} รายการ</h2><SmartTable data={paginatedData} columns={columns} isLoading={isLoading} pagination={{ pageIndex: filters.page, pageSize: filters.limit, totalCount: filteredData.length, onPageChange: handlePageChange, onPageSizeChange: (size) => setFilters({ limit: size, page: 1 }) }} rowIdField="pattern_id" /></div>
            <PatternFormModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingId(null); }} editId={editingId} onSuccess={fetchData} />
        </div>
    );
}
