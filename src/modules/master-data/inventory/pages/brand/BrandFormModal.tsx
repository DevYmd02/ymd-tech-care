...
export function BrandFormModal({ isOpen, onClose, editId, initialData, onSuccess }: Props) {
    const {
        register,
        errors,
        isSaving,
        handleSave,
        clearForm
    } = useBrandForm(editId || null, initialData, onSuccess);
    ...
    return (
        <DialogFormLayout
            isOpen={isOpen}
            onClose={handleClose}
            title={editId ? 'แก้ไขข้อมูลยี่ห้อสินค้า' : 'เพิ่มยี่ห้อสินค้าใหม่'}
            titleIcon={TitleIcon}
            footer={FormFooter}
        >
            <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-end pb-2">
                        <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700 w-full">
                            ...
                            />
                            <label htmlFor="brand_is_active" className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                                สถานะใช้งาน (Active)
                            </label>
                        </div>
                    </div>
                ...
                {/* Brand Name (Thai) */}
                <div>
                    <label className={styles.label}>ชื่อยี่ห้อ (ไทย) <span className="text-red-500">*</span></label>
                    <input {...register('nameTh')} type="text" placeholder="กรอกชื่อยี่ห้อ (ไทย)" className={`${styles.input} ${errors.nameTh ? 'border-red-500 focus:ring-red-200' : ''}`} />
                    {errors.nameTh && <p className="text-red-500 text-xs mt-1">{errors.nameTh.message}</p>}
                </div>
                {/* Brand Name (English) */}
                <div>
                    <label className={styles.label}>ชื่อยี่ห้อ (EN)</label>
                    <input {...register('nameEn')} type="text" placeholder="กรอกชื่อยี่ห้อ (English)" className={`${styles.input} ${errors.nameEn ? 'border-red-500' : ''}`} />
