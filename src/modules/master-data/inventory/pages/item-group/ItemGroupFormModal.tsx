...
import { useItemGroupForm } from '../../hooks/useItemGroupForm';
...
export function ItemGroupFormModal({ isOpen, onClose, editId, initialData, onSuccess }: Props) {
    const {
        register,
        errors,
        isSaving,
        handleSave,
        clearForm
    } = useItemGroupForm(editId ?? null, initialData, onSuccess);
    ...
    return (
        <DialogFormLayout isOpen={isOpen} onClose={handleClose} title={editId ? 'แก้ไขข้อมูลกลุ่มสินค้า' : 'เพิ่มกลุ่มสินค้าใหม่'} titleIcon={TitleIcon} footer={FormFooter}>
            <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-end pb-2">
                        <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700 w-full">
                            <input
                                {...register('isActive')}
                                type="checkbox"
                                id="item_group_is_active"
                                className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer"
                            />
                            <label htmlFor="item_group_is_active" className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                                สถานะใช้งาน (Active)
                            </label>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <input {...register('nameEn')} type="text" placeholder="กรอกชื่อ (English)" className={`${styles.input} ${errors.nameEn ? 'border-red-500' : ''}`} />
