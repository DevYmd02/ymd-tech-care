...
export function useShelfForm(editId: number | null, initialData?: Shelf | null, onSuccess?: () => void) {
    ...
    const { data: duplicateCheckData } = useQuery({
        queryKey: ['shelf-check-duplicate', debouncedCode],
        queryFn: async () => {
            if (!debouncedCode) return { items: [] };
            return ShelfService.getAll({ code: debouncedCode });
        },
    ...
    useEffect(() => {
        if (duplicateCheckData?.items && debouncedCode) {
            ...
            if (isDuplicate) {
                setError('code', { type: 'manual', message: 'รหัสชั้นวางสินค้าซ้ำในระบบ' });
            } else if (errors.code?.message === 'รหัสชั้นวางสินค้าซ้ำในระบบ') {
                clearErrors('code');
            }
        }
    ...
    const saveMutation = useMutation({
        ...
        onError: async (error: Error) => {
            ...
            if (isDuplicate) {
                setError('code', { message: 'รหัสชั้นวางสินค้าซ้ำในระบบ' });
                return;
            }
