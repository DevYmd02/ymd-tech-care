...
export const modelSchema = z.object({
    code: z.string().min(1, 'กรุณากรอกรหัสรุ่นสินค้า').max(20, 'รหัสต้องไม่เกิน 20 ตัวอักษร'),
    nameTh: z.string().min(1, 'กรุณากรอกชื่อรุ่นสินค้า (ไทย)').max(200, 'ชื่อต้องไม่เกิน 200 ตัวอักษร'),
...
const initialFormData: ModelFormValues = {
    code: '',
    nameTh: '',
    nameEn: '',
    isActive: true,
...
export function useModelForm(editId: number | null, initialData?: Model | null, onSuccess?: () => void) {
    ...
    const { data: duplicateCheckData } = useQuery({
        queryKey: ['model-check-duplicate', debouncedCode],
        queryFn: async () => {
            if (!debouncedCode) return { items: [] };
            return ModelService.getAll({ code: debouncedCode });
        },
    ...
    useEffect(() => {
        if (duplicateCheckData?.items && debouncedCode) {
            ...
            if (isDuplicate) {
                setError('code', { type: 'manual', message: 'รหัสรุ่นสินค้าซ้ำในระบบ' });
            } else if (errors.code?.message === 'รหัสรุ่นสินค้าซ้ำในระบบ') {
                clearErrors('code');
            }
        }
    ...
    const saveMutation = useMutation({
        mutationFn: (data: ModelFormValues) => {
            return editId 
                ? ModelService.update(editId, data)
                : ModelService.create(data);
        },
        onSuccess: async (res) => {
            if (res.success) {
                await confirm({ title: 'บันทึกสำเร็จ!', description: 'ข้อมูลรุ่นสินค้าถูกบันทึกเรียบร้อยแล้ว', confirmText: 'ตกลง', variant: 'success', hideCancel: true });
        ...
        onError: async (error: Error) => {
            logger.error('Save model error:', error);
            const errorMsg = error.message.toLowerCase();
            const isDuplicate = errorMsg.includes('duplicate') || errorMsg.includes('ซ้ำ');
            
            if (isDuplicate) {
                setError('code', { message: 'รหัสรุ่นสินค้าซ้ำในระบบ' });
                return;
            }
    ...
    const handleSave: SubmitHandler<ModelFormValues> = (data) => saveMutation.mutate(data);
